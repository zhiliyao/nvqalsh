...
NVMAllocator::NVMAllocator(const char* data_file, const size_t size) {
    size_ = size;
    size_t mapped_len;
    int is_pmem;
    ...
    //     remove(data_file);
    // }
    nvm_pool = pmem_map_file(data_file, size, PMEM_FILE_CREATE, 0666, 
                             &mapped_len, &is_pmem);
    metadata = new allocator_meta();
    // metadata = (allocator_meta*)nvm_pool;
    // memcpy(pool_name, data_file, strlen(data_file));
    // if (metadata->cur_offset == 0) {
    //     metadata->cur_offset += sizeof(allocator_meta);
    //     metadata->end_offset = size;
    // }
    metadata->cur_offset = 0;
    metadata->end_offset = size;
}

void* NVMAllocator::PMAlloc(size_t size) {
    if (metadata->cur_offset + size <= metadata->end_offset) {
        register void *p;
        p = (nvm_pool + metadata->cur_offset);
        metadata->cur_offset += size;
        return p;
    }
...
void* NVMAllocator::getVirtualAddr(uint64_t ppointer) {
    if (ppointer == 0) return NULL;
    return nvm_pool + ppointer;
}

uint64_t NVMAllocator::getPPointer(void* virtual_addr) {
    if (virtual_addr < nvm_pool || 
        virtual_addr >= (nvm_pool + metadata->end_offset)) {
        return 0;
    }
    return (uint64_t)virtual_addr - (uint64_t)nvm_pool;
}

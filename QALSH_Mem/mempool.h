...
class NVMAllocator {
private:
    allocator_meta* metadata;
    void* nvm_pool;
    char pool_name[64];
    size_t size_;
public:
    NVMAllocator(const char* data_file, const size_t size);

    void* PMAlloc(size_t size);
    void* getVirtualAddr(uint64_t ppointer);
    uint64_t getPPointer(void* virtual_addr);
    ~NVMAllocator() {
      pmem_unmap(nvm_pool, size_);
    }
};
...
// Jiaqiao Zhang
extern NVMAllocator *nvm_allocator;
...
// Jiaqiao Zhang
#define nvmpool_alloc nvm_allocator->PMAlloc
